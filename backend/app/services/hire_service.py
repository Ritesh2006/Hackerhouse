    ...
    async def hire_developer(self, client_id: str, hire_data: ProjectCreate):
        try:
            ...
            try:
                ...
                sync_res = {"synced": False, "error": str(le)}
            ...
            await self.chat_repo.create({
                ...
                "messages": [
                    {
                        "sender_id": client_id,
                        "text": f"Hi {dev_name}, I have proposed a new contract for '{hire_data.title}' with a budget of ${hire_data.budget}. Details: {hire_data.description}",
                        "timestamp": now,
                        "linkedin_status": sync_res
                    }
                ],
                "created_at": now,
                "updated_at": now
